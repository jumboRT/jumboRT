...
void
	interior_node_init(t_node_info *parent_info, const t_node_info *above_info, const t_split *split) {
			...
			t_tree_edges *edges)
{
	...
	while (index < edges->count)
	{
		...
		{
			node->a.inline_primitives[offset]
	...
	leaf_node_init(t_node_info *node_info)
{
	...
	if (primitive_count <= ACCEL_INLINE_PRIMS)
		...
		node->a.primitive_offset = push_back_indices(node_info->tree,
				node_info->edges);
}

static inline float calculate_cost(float total_sa, const float sub_sa[2],
...
static void get_surface_areas(const t_bounds bounds, const float offset,
