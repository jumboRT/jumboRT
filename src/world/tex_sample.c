...
static t_vec
	sample_texture(const GLOBAL t_world *world, t_tex tex, t_vec2 uv, t_vec2 offset)
{
	uint64_t					xv;
	uint64_t					yv;
	...
	xv = ((uint64_t)(u(uv) * tex.a.tex.width) + (uint64_t) u(offset)) % tex.a.tex.width;
	yv = ((uint64_t)(v(uv) * tex.a.tex.height) + (uint64_t) v(offset)) % tex.a.tex.height;
...
t_vec
	sample_color_offset(const GLOBAL t_world *world, uint32_t id, t_vec2 uv, t_vec2 offset)
{
	t_vec	v;
