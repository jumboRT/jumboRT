...
t_vec
	tex_sample_id_offset(const GLOBAL t_world *world, uint32_t texidx, t_vec2 uv, t_vec2 poffset)
{
	...
	if (tex->type == RT_TEX_COLOR)
		return (tex->a.color);
	return tex_sample_offset(world, *tex, uv, poffset);
}

t_vec
	tex_sample_id(const GLOBAL t_world *world, uint32_t texidx, t_vec2 uv)
{
	return (tex_sample_id_offset(world, texidx, uv, vec2(0.0, 0.0)));
}
...
t_vec
	tex_sample_offset(const GLOBAL t_world *world, t_tex tex, t_vec2 uv, t_vec2 poffset)
{
	uint64_t					xv;
	uint64_t					yv;
...
t_vec
	tex_sample(const GLOBAL t_world *world, t_tex tex, t_vec2 uv)
{
